...
def chooseVote(drop_button,dropbutton2,inputbox, text, btn):
    ...
    ubsig = elgamalblind.unblind(signed_blind,pubkey,k,r,h,blind_commit)
    print(f'ubsig is : {ubsig}')
    print(f"r is : {r}")
    ecommit = commit.encode("utf-8")
    print(ecommit)
    elgamalblind.verefy(m,ubsig,pubkey,r)
    contract_instance.functions.sendvote(commit.encode(),m,ubsig,r).transact()
